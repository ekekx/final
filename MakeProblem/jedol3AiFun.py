    ...
    match ai_mode:
        case "jedolGPT":
            ...
            prompt.append({"role": "system", "content": f"""
                                ...
                                질문에 간결하게 답한다.
                                """})
            prompt.append({"role": "system", "content": f"문제와 함께 정답을 제시해라" })
            prompt=chat_history+ prompt 
             
            prompt.append({"role": "user", "content": query } )    
            
            prompt.append({"role": "user", "content": f"객관식 문제를 만들어줘" })
            prompt.append({"role": "user", "content": f"서술형 문제를 만들어줘" })
            prompt.append({"role": "user", "content": f"빈 칸 문제를 만들어줘" })
